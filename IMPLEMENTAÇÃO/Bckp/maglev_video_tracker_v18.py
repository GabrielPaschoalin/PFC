...
# -*- coding: utf-8 -*-
"""
MAGLEV VIDEO TRACKER V18 - SIMPLE DARK-BLOB TOP TRACKER + ROBUST SHAPE FILTER
...
Exemplo:
    python maglev_video_tracker_v18.py --video video.mp4 --output-dir saida_v18
"""
...
class Params:
    ...
    ball_dark_threshold: int = 135
    ball_dark_percentile: float = 8.0
    # Protecao contra percentil escuro baixo demais: se o percentil cair muito,
    # ele captura so sombras/fendas e a bolinha desaparece.
    # A V18 so deixa o percentil apertar a mascara ate este piso.
    ball_min_effective_threshold: int = 95
    ...
    # Rejeita faixas alongadas da base/sombra quando a bolinha levita.
    ball_min_aspect_ratio: float = 0.45
    ball_max_aspect_ratio: float = 2.25
...
def make_dark_ball_mask(frame: np.ndarray, ruler: dict, p: Params) -> np.ndarray:
    ...
    thr = abs_thr
    if valid_pixels.size and float(p.ball_dark_percentile) > 0:
        qthr = float(np.percentile(valid_pixels, float(p.ball_dark_percentile)))
        # Se o percentil fica muito baixo, ele representa a fenda/sombra da base,
        # nao a bolinha. Nesse caso mantem o threshold absoluto.
        floor_thr = float(getattr(p, "ball_min_effective_threshold", 95))
        if floor_thr <= qthr <= abs_thr:
            thr = qthr
...
def detect_ball_candidates(frame: np.ndarray, ruler: dict, p: Params) -> Tuple[List[dict], np.ndarray]:
    ...
    for contour in contours:
        ...
        if aspect < float(getattr(p, "ball_min_aspect_ratio", 0.45)) or aspect > float(getattr(p, "ball_max_aspect_ratio", 2.25)):
            continue
        ...
        darkness = float(np.clip((170.0 - mean_gray) / 170.0, 0.0, 1.0))
        ...
        aspect_centered = abs(math.log(max(1e-6, aspect)))
        aspect_limit = max(1e-6, math.log(float(getattr(p, "ball_max_aspect_ratio", 2.25))))
        aspect_score = float(np.clip(1.0 - aspect_centered / aspect_limit, 0.0, 1.0))
        ...
        score = 0.26 * area_score + 0.28 * darkness + 0.18 * aspect_score + 0.12 * circ_score + 0.10 * fill_score + 0.06
        cand = {
            "x_center_px": cx,
            "y_center_px": cy,
            "bbox_x_px": float(x),
...
def process_video(video_path: str, output_dir: str, p: Params):
    ...
    print("MAGLEV VIDEO TRACKER V18")
...
def make_plot(df: pd.DataFrame, out_path: str):
    ...
    ax.set_title(f"Maglev video tracking V18 | time_conf={time_conf}")
...
def main():
    ...
    ap.add_argument("--ball-dark-threshold", type=int, default=None, help="Threshold absoluto para bolinha escura. Default: 135")
    ap.add_argument("--ball-dark-percentile", type=float, default=None, help="Percentil escuro do frame. Default: 8")
    ap.add_argument("--ball-min-effective-threshold", type=int, default=None, help="Piso do threshold efetivo quando usa percentil; evita perder a bolinha para sombras")
    ap.add_argument("--ball-min-area-px", type=int, default=None, help="Area minima do blob da bolinha")
    ap.add_argument("--ball-min-aspect-ratio", type=float, default=None, help="Aspect ratio minimo do bbox da bolinha")
    ap.add_argument("--ball-max-aspect-ratio", type=float, default=None, help="Aspect ratio maximo do bbox da bolinha")
    ...
    if args.ball_min_effective_threshold is not None:
        p.ball_min_effective_threshold = int(args.ball_min_effective_threshold)
    if args.ball_min_area_px is not None:
        p.ball_min_area_px = int(args.ball_min_area_px)
    if args.ball_min_aspect_ratio is not None:
        p.ball_min_aspect_ratio = float(args.ball_min_aspect_ratio)
