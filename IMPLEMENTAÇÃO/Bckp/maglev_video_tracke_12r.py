...
# -*- coding: utf-8 -*-
"""
MAGLEV VIDEO TRACKER V12 - SYNC-FIRST + SUBPIXEL EDGE METROLOGY
...
Objetivo:
    ...
    V12: reorganiza o pipeline para sincronizar antes do tracking, separa ROIs,
    adiciona medição metrológica do topo por perfil de borda subpixel e
    torna o tempo seguro quando o pulso visual é curto demais.
...
Exemplo:
    python maglev_video_tracker_v12.py --video ident_slow_mo.mp4
"""
...
class Params:
    ...
    # Metrologia V12
    # -------------------------
    sync_first_tracking: bool = True
    sync_pre_roll_frames: int = 0
    sync_min_pulse_width_frames_for_fps: int = 10
    object_polarity: str = "auto"  # "auto", "bright" ou "dark"
    dark_percentile: float = 38.0
    template_match_enabled: bool = True
    template_match_threshold: float = 0.45
    edge_subpixel_enabled: bool = True
    edge_search_margin_ratio: float = 0.52
    edge_x_band_ratio: float = 0.62
    edge_min_gradient: float = 6.0
    edge_top_percentile: float = 18.0
    edge_min_points: int = 10
    edge_temporal_median_window: int = 7
    edge_draw_on_preview: bool = True
...
def flag_physical_outliers(df: pd.DataFrame, p: Params) -> pd.DataFrame:
    ...
    candidate_cols = [
        "z_top_edge_mm_corr_median",
        "z_top_edge_mm_corr",
        "z_top_fixed_mm_rel_corr_median",
        "z_top_fixed_mm_rel_corr",
        "z_top_fixed_mm_rel",
        "y_top_edge_subpx_median",
        "y_top_edge_subpx",
        "y_top_fixed_px",
    ]
...
def _add_template_candidate(
    ...
    if template is None or not getattr(p, "template_match_enabled", True):
        return
    if template.size == 0 or gray.size == 0:
        return
    th, tw = template.shape[:2]
    if gray.shape[0] < th + 2 or gray.shape[1] < tw + 2:
        return
    try:
        result = cv2.matchTemplate(gray, template, cv2.TM_CCOEFF_NORMED)
        _, max_val, _, max_loc = cv2.minMaxLoc(result)
        if float(max_val) < float(getattr(p, "template_match_threshold", 0.45)):
            return
        x, y = max_loc
        cx_roi = x + 0.5 * tw
        cy_roi = y + 0.5 * th
        rr = 0.25 * (tw + th)
        ox, oy = roi_offset
        local_diff = diff_blur[max(0, y):min(diff_blur.shape[0], y + th), max(0, x):min(diff_blur.shape[1], x + tw)]
        motion = float(np.mean(local_diff)) if local_diff.size else 0.0
        score = float(np.clip(0.55 * max_val + 0.45 * min(1.0, motion / 80.0), 0.0, 1.0))
        candidates.append({
            "x_center_px": float(cx_roi + ox),
            "y_center_px": float(cy_roi + oy),
            "radius_px": float(rr),
            "x_left_px": float(cx_roi + ox - rr),
            "y_top_px": float(cy_roi + oy - rr),
            "x_right_px": float(cx_roi + ox + rr),
            "y_bottom_px": float(cy_roi + oy + rr),
            "confidence": score,
            "method": "template",
            "area": float(math.pi * rr * rr),
            "circularity": 0.75,
            "fill_ratio": 0.75,
            "brightness": float(np.mean(gray[max(0, y):min(gray.shape[0], y + th), max(0, x):min(gray.shape[1], x + tw)])),
            "motion": motion,
...
def detect_candidates_in_frame(frame: np.ndarray, bg_gray: np.ndarray, roi: Tuple[int, int, int, int], template: Optional[np.ndarray], p: Params, gpu: Optional[FrameGpuAccelerator] = None) -> List[dict]:
    ...
    dark_thr = np.percentile(gray_blur, float(getattr(p, "dark_percentile", 38.0)))
    ...
    polarity = str(getattr(p, "object_polarity", "auto")).lower()
    if polarity == "bright":
        appearance_mask = bright_mask
    elif polarity == "dark":
        appearance_mask = dark_mask
    else:
        appearance_mask = cv2.bitwise_or(bright_mask, dark_mask)

    mask = cv2.bitwise_and(appearance_mask, cv2.dilate(diff_mask, None, iterations=1))
    ...
    _add_template_candidate(candidates, gray, diff_blur, (x0, y0), template, p)
    ...
    try:
        ...
        if circles is None:
            circles_cpu = cv2.HoughCircles(
                ...
                minDist=max(20, p.min_ball_radius_px), param1=80, param2=18,
                minRadius=p.min_ball_radius_px, maxRadius=p.max_ball_radius_px,
            )
        ...
        if circles is not None and len(circles) > 0:
            for c in np.round(circles).astype(int)[:10]:
                ...
                appearance_score = max(bright_score, dark_score) if polarity == "auto" else (dark_score if polarity == "dark" else bright_score)
                motion_score = min(1.0, mean_diff / 80.0)
                ...
                score = 0.38*motion_score + 0.22*appearance_score + 0.25*size_score + 0.15
                candidates.append({
                    "x_center_px": float(cxr + x0), "y_center_px": float(cyr + y0), "radius_px": float(rr),
                    "x_left_px": float(cxr + x0 - rr), "y_top_px": float(cyr + y0 - rr),
                    "x_right_px": float(cxr + x0 + rr), "y_bottom_px": float(cyr + y0 + rr),
                    "confidence": float(score), "method": "hough", "area": float(math.pi*rr*rr),
                    "circularity": 1.0, "fill_ratio": 0.78, "brightness": mean_bright,
                    "motion": mean_diff, "template_score": 0.0,
                })
    except Exception:
        pass
...
def add_time_and_sync_columns(df: pd.DataFrame, meta: Dict[str, object], sync_df: pd.DataFrame, p: Params) -> Tuple[pd.DataFrame, int, int]:
    ...
    # Se o metadado não trouxe FPS de captura confiável, só usa a largura do pulso
    # conhecido do Arduino se a borda visual tiver amostras suficientes.
    pulse_frames_for_fps = (fall_frame - rise_frame) if (rise_frame >= 0 and fall_frame > rise_frame) else 0
    min_pulse_frames = int(getattr(p, "sync_min_pulse_width_frames_for_fps", 10))
    sync_pulse_width_usable_for_fps = pulse_frames_for_fps >= min_pulse_frames
    if confidence == "LOW" and sync_pulse_width_usable_for_fps and p.sync_known_pulse_width_ms > 0:
        fps_from_pulse = 1000.0 * float(pulse_frames_for_fps) / float(p.sync_known_pulse_width_ms)
        if fps_from_pulse > 1:
            fps_capture = fps_from_pulse
            confidence = "PULSE_ESTIMATED"
            source = "sync_led_known_width"
    elif confidence == "LOW" and rise_frame >= 0 and fall_frame > rise_frame and not sync_pulse_width_usable_for_fps:
        confidence = "LOW_SYNC_WIDTH"
    ...
    df["sync_pulse_width_usable_for_fps"] = bool(sync_pulse_width_usable_for_fps)
    df["sync_min_pulse_width_frames_for_fps"] = int(getattr(p, "sync_min_pulse_width_frames_for_fps", 10))
...
def collect_sync_rows_first(video_path: str, p: Params, gpu: Optional[FrameGpuAccelerator] = None) -> Tuple[pd.DataFrame, int, int]:
    rows = []
    if not getattr(p, "sync_enabled", True):
    ...
    return finalize_sync_dataframe(sync_df, p)


def _empty_track_row(i: int, fps: float) -> dict:
...
def build_tracking_dataframe_from_track(track: List[Optional[dict]], frame_ids: List[int], total_frames: int, fps: float, p: Params) -> pd.DataFrame:
    rows = [_empty_track_row(i, fps) for i in range(total_frames)]
    for local_i, cand in enumerate(track):
        if local_i >= len(frame_ids):
            break
        i = int(frame_ids[local_i])
        if i < 0 or i >= total_frames or cand is None:
            continue
        time_video_s = i / fps if fps else np.nan
        rows[i] = {
            "frame": i,
...
def add_fixed_radius_columns(df: pd.DataFrame, p: Params) -> Tuple[pd.DataFrame, float]:
    df = df.copy()
    ...
    if fixed_radius_px <= 0 and p.auto_fixed_radius:
        good_radius = df.loc[(df["confidence"] >= p.radius_median_min_confidence) & df["radius_px"].notna(), "radius_px"]
    ...
    return df, fixed_radius_px


def apply_manual_scale_fallback(df: pd.DataFrame, p: Params) -> pd.DataFrame:
    df = df.copy()
    auto_ok = bool(df["auto_ruler_ok"].iloc[0]) if len(df) and "auto_ruler_ok" in df.columns else False
    if auto_ok:
        return df
...
def _subpixel_peak_offset(gm: float, g0: float, gp: float) -> float:
    denom = gm - 2.0 * g0 + gp
    if abs(denom) < 1e-9:
        return 0.0
    return float(np.clip(0.5 * (gm - gp) / denom, -0.75, 0.75))


def estimate_top_edge_subpixel(frame: np.ndarray, row: pd.Series, p: Params) -> dict:
    if not bool(row.get("valid", False)):
        return {"edge_valid": False, "edge_reason": "invalid_track"}
    cx = float(row.get("x_center_px", np.nan))
    cy = float(row.get("y_center_px", np.nan))
    r = float(row.get("fixed_radius_px", row.get("radius_px", np.nan)))
    if not (np.isfinite(cx) and np.isfinite(cy) and np.isfinite(r) and r > 4):
        return {"edge_valid": False, "edge_reason": "bad_geometry"}

    H, W = frame.shape[:2]
    y_pred = cy - r
    margin = max(6, int(round(float(getattr(p, "edge_search_margin_ratio", 0.52)) * r)))
    half_band = max(6, int(round(float(getattr(p, "edge_x_band_ratio", 0.62)) * r)))
    x1 = max(0, int(round(cx - half_band)))
    x2 = min(W, int(round(cx + half_band)) + 1)
    y1 = max(0, int(round(y_pred - margin)))
    y2 = min(H, int(round(y_pred + margin)) + 1)
    if x2 - x1 < 5 or y2 - y1 < 7:
        return {"edge_valid": False, "edge_reason": "small_window"}

    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    patch = gray[y1:y2, x1:x2].astype(np.float32)
    patch = cv2.GaussianBlur(patch, (3, 3), 0)
    k = max(7, (patch.shape[0] // 2) * 2 + 1)
    patch_norm = patch - cv2.GaussianBlur(patch, (1, k), 0)
    gy = cv2.Sobel(patch_norm, cv2.CV_32F, 0, 1, ksize=3)
    abs_gy = np.abs(gy)

    ys, xs, strengths = [], [], []
    min_grad = float(getattr(p, "edge_min_gradient", 6.0))
    for col in range(abs_gy.shape[1]):
        prof = abs_gy[:, col]
        if len(prof) < 3:
            continue
        yi = int(np.argmax(prof))
        g0 = float(prof[yi])
        if g0 < min_grad or yi <= 0 or yi >= len(prof) - 1:
            continue
        off = _subpixel_peak_offset(float(prof[yi - 1]), g0, float(prof[yi + 1]))
        ys.append(y1 + yi + off)
        xs.append(x1 + col)
        strengths.append(g0)

    min_points = int(getattr(p, "edge_min_points", 10))
    if len(ys) < min_points:
        return {"edge_valid": False, "edge_reason": "few_edge_points", "edge_points_used": len(ys)}
    ys = np.asarray(ys, dtype=float)
    xs = np.asarray(xs, dtype=float)
    strengths = np.asarray(strengths, dtype=float)
    med = np.median(ys)
    mad = np.median(np.abs(ys - med)) + 1e-6
    keep = np.abs(ys - med) <= max(3.0, 2.5 * 1.4826 * mad)
    ys, xs, strengths = ys[keep], xs[keep], strengths[keep]
    if len(ys) < min_points:
        return {"edge_valid": False, "edge_reason": "few_robust_points", "edge_points_used": len(ys)}
    top_percentile = float(getattr(p, "edge_top_percentile", 18.0))
    y_top = float(np.percentile(ys, top_percentile))
    near = ys <= np.percentile(ys, min(45.0, top_percentile + 20.0))
    x_edge = float(np.median(xs[near])) if np.any(near) else float(np.median(xs))
    rmse = float(np.sqrt(np.mean((ys - np.median(ys)) ** 2)))
    conf_grad = float(np.clip((np.median(strengths) - min_grad) / max(1.0, 4.0 * min_grad), 0.0, 1.0))
    conf_n = float(np.clip(len(ys) / max(1.0, 0.5 * (x2 - x1)), 0.0, 1.0))
    confidence = float(np.clip(0.65 * conf_grad + 0.35 * conf_n, 0.0, 1.0))
    return {"edge_valid": True, "edge_reason": "OK", "y_top_edge_subpx": y_top,
            "x_top_edge_px": x_edge, "edge_confidence": confidence,
            "edge_points_used": int(len(ys)), "edge_fit_rmse_px": rmse,
            "edge_gradient_median": float(np.median(strengths))}


def add_subpixel_edge_measurements(video_path: str, df: pd.DataFrame, ruler: dict, p: Params) -> pd.DataFrame:
    df = df.copy()
    for k, v in {"edge_valid": False, "edge_reason": "not_run", "y_top_edge_subpx": np.nan,
                 "x_top_edge_px": np.nan, "edge_confidence": np.nan, "edge_points_used": 0,
                 "edge_fit_rmse_px": np.nan, "edge_gradient_median": np.nan,
                 "y_top_edge_ruler_mm_corr": np.nan, "z_top_edge_mm_corr": np.nan}.items():
        df[k] = v
    if not getattr(p, "edge_subpixel_enabled", True):
        df["edge_reason"] = "disabled"
        return df
    valid_frames = set(df.loc[df["valid"].astype(bool), "frame"].astype(int).tolist()) if "valid" in df.columns else set()
    if not valid_frames:
        return df
    frame_to_index = {int(f): idx for idx, f in zip(df.index, df["frame"].astype(int))}
    for frame_idx, frame in iter_video_frames(video_path, p):
        if int(frame_idx) not in valid_frames:
            continue
        idx = frame_to_index.get(int(frame_idx))
        if idx is None:
            continue
        res = estimate_top_edge_subpixel(frame, df.loc[idx], p)
    ...
    if ruler.get("auto_ruler_ok", False):
        ...
        u = np.array([ruler["ruler_u_x"], ruler["ruler_u_y"]], dtype=float)
        mm_per_px = float(ruler["mm_per_px_ruler"])
        pts = np.column_stack([df["x_top_edge_px"].to_numpy(dtype=float), df["y_top_edge_subpx"].to_numpy(dtype=float)])
        s_edge = (pts - origin) @ u
        df["y_top_edge_ruler_mm_corr"] = s_edge * mm_per_px
        valid_mask = df["edge_valid"].astype(bool) & df["y_top_edge_ruler_mm_corr"].notna()
        ref_idx = None
        if "sync_rising_frame" in df.columns and df["sync_rising_frame"].notna().any():
            rise_frame = int(df["sync_rising_frame"].dropna().iloc[0])
            candidates = df.index[valid_mask & (df["frame"] >= rise_frame)]
            if len(candidates):
                ref_idx = candidates[0]
        if ref_idx is None:
            candidates = df.index[valid_mask]
            if len(candidates):
                ref_idx = candidates[0]
        if ref_idx is not None:
            ref_edge = float(df.loc[ref_idx, "y_top_edge_ruler_mm_corr"])
            df["z_top_edge_mm_corr"] = ref_edge - df["y_top_edge_ruler_mm_corr"]
    win = int(getattr(p, "edge_temporal_median_window", 7))
    if win > 1:
        if win % 2 == 0:
            win += 1
        for col in ["y_top_edge_subpx", "z_top_edge_mm_corr"]:
            if col in df.columns and df[col].notna().any():
                df[col + "_median"] = df[col].rolling(win, center=True, min_periods=1).median()
    return df


def add_median_columns(df: pd.DataFrame, p: Params) -> pd.DataFrame:
    df = df.copy()
    if p.median_smooth_window and p.median_smooth_window > 1:
        win = int(p.median_smooth_window)
        if win % 2 == 0:
            win += 1
        for col in ["x_center_px", "y_center_px", "radius_px", "y_top_px", "y_top_fixed_px",
...
def process_video(
    ...
    sync_df, rise_frame, fall_frame = collect_sync_rows_first(video_path, p, gpu)
    track_start_frame = 0
    if getattr(p, "sync_first_tracking", True) and rise_frame >= 0:
        track_start_frame = max(0, int(rise_frame) - int(getattr(p, "sync_pre_roll_frames", 0)))
    ...
    template = None
    for frame_idx, frame in iter_video_frames(video_path, p):
        if int(frame_idx) < track_start_frame:
            continue
        ...
        processed_frame_ids.append(int(frame_idx))
        if debug_candidates:
            for rank, c in enumerate(cands):
                row = {"frame": int(frame_idx), "rank": rank}
                row.update(c)
                candidate_rows.append(row)

    track = choose_track_dp(all_candidates, p)
    df = build_tracking_dataframe_from_track(track, processed_frame_ids, n, fps, p)
    df, fixed_radius_px = add_fixed_radius_columns(df, p)
    if sync_df.empty:
        sync_df = pd.DataFrame({"frame": df["frame"]})
    ...
    df = apply_manual_scale_fallback(df, p)
    df = add_subpixel_edge_measurements(video_path, df, ruler, p)
    df = add_median_columns(df, p)
    ...
    df["roi_w_px"] = roi[2]
    df["roi_h_px"] = roi[3]
    df["track_start_frame"] = int(track_start_frame)
    df["sync_first_tracking"] = bool(getattr(p, "sync_first_tracking", True))
    df["video_fps"] = fps
    ...
    print("MAGLEV VIDEO TRACKER V12")
    ...
    print(f"Track start frame: {track_start_frame}")
    ...
    if len(df) and df["sync_pulse_width_ms"].notna().any():
        ...
        print(f"Sync pulse usable for FPS: {bool(df['sync_pulse_width_usable_for_fps'].iloc[0])}")
    if ruler.get("auto_ruler_ok", False):
    ...
    if "edge_valid" in df.columns:
        print(f"Subpixel edge valid full: {int(df['edge_valid'].sum())}/{len(df)}")
        print(f"Subpixel edge valid treated: {int(treated['edge_valid'].sum())}/{len(treated)}")
    if df["y_center_px"].notna().any():
        ...
        if "y_top_edge_subpx" in df.columns and df["y_top_edge_subpx"].notna().any():
            print(f"y_top_edge_subpx range: {df['y_top_edge_subpx'].min():.2f} .. {df['y_top_edge_subpx'].max():.2f}")
...
def make_preview(video_path: str, df: pd.DataFrame, roi: Tuple[int, int, int, int], out_path: str, p: Params):
    ...
    for idx in idxs:
        ...
        if bool(row["valid"]) and not np.isnan(row["x_center_px"]):
            ...
            if getattr(p, "edge_draw_on_preview", True) and "y_top_edge_subpx" in row and not np.isnan(row["y_top_edge_subpx"]):
                ye = int(round(row["y_top_edge_subpx"]))
                xe = int(round(row["x_top_edge_px"])) if "x_top_edge_px" in row and not np.isnan(row["x_top_edge_px"]) else cx
                cv2.line(vis, (max(0, xe-rf), ye), (min(vis.shape[1]-1, xe+rf), ye), (0, 165, 255), 2)
                text = f"f={idx} edge={row['y_top_edge_subpx']:.1f} conf={row['confidence']:.2f}"
            else:
...
def make_plot(df: pd.DataFrame, out_path: str):
    ...
    ax = plt.gca()
    time_conf = str(df["metadata_time_confidence"].iloc[0]) if "metadata_time_confidence" in df.columns and len(df) else ""
    ...
    valid = df["valid_signal"].astype(bool) if "valid_signal" in df.columns else (df["valid"].astype(bool) if "valid" in df.columns else pd.Series(True, index=df.index))
    y_candidates = [
        ("z_top_edge_mm_corr_median", "z_top_edge_mm_corr smooth"),
        ("z_top_edge_mm_corr", "z_top_edge_mm_corr raw"),
        ("z_top_fixed_mm_rel_corr_median", "z_top_fixed_mm_rel_corr smooth"),
        ("z_top_fixed_mm_rel_corr", "z_top_fixed_mm_rel_corr"),
        ("z_top_fixed_mm_rel", "z_top_fixed_mm_rel"),
        ("y_top_edge_subpx_median", "y_top_edge_subpx smooth"),
        ("y_top_edge_subpx", "y_top_edge_subpx"),
        ("y_top_fixed_px", "y_top_fixed_px"),
        ("y_center_px", "y_center_px"),
    ]
    plotted = False
    for col, label in y_candidates:
        if col in df.columns and df[col].notna().any():
            ax.plot(x, df[col].where(valid), label=label, linewidth=1.5)
            plotted = True
            break
    if "z_center_mm_rel_corr_median" in df.columns and df["z_center_mm_rel_corr_median"].notna().any():
        ax.plot(x, df["z_center_mm_rel_corr_median"].where(valid), label="z_center_mm_rel_corr smooth", linewidth=0.9, alpha=0.65)
    elif "z_center_mm_rel_corr" in df.columns and df["z_center_mm_rel_corr"].notna().any():
        ax.plot(x, df["z_center_mm_rel_corr"].where(valid), label="z_center_mm_rel_corr", linewidth=0.9, alpha=0.65)
    if "sync_falling_edge" in df.columns:
        fall_rows = df.index[df["sync_falling_edge"].fillna(0).astype(int) == 1]
        if len(fall_rows):
            fall_x = float(x.loc[fall_rows[0]]) if hasattr(x, "loc") else float(x.iloc[fall_rows[0]])
            ax.axvline(fall_x, linestyle=":", linewidth=1.0, label="FALLING_EDGE")
    if "sync_rising_edge" in df.columns:
        if xlabel.startswith("time_sync"):
            ax.axvline(0.0, linestyle="--", linewidth=1.0, label="RISING_EDGE")
        else:
            rise_rows = df.index[df["sync_rising_edge"].fillna(0).astype(int) == 1]
            if len(rise_rows):
                rise_x = float(x.loc[rise_rows[0]]) if hasattr(x, "loc") else float(x.iloc[rise_rows[0]])
                ax.axvline(rise_x, linestyle="--", linewidth=1.0, label="RISING_EDGE")
    ax.set_ylabel("deslocamento corrigido (mm) / posição" if plotted else "posição")
    ax.set_xlabel(xlabel)
    ax.set_title(f"Maglev video tracking V12 | time_conf={time_conf}")
