...
# -*- coding: utf-8 -*-
"""
MAGLEV VIDEO TRACKER V31 - GREEN LINE HEIGHT RULER REFERENCE + LOWER MIN TOPO
...
Exemplo:
    python maglev_video_tracker_v31.py --video video.mp4 --output-dir saida_v31
"""
...
class Params:
    ...
    # A régua começa em zero embaixo. Valores menores que ~0,9 cm são fisicamente
    # suspeitos para o topo da bolinha nos ensaios atuais e indicam fenda/base.
    ball_min_topo_ruler_mm: float = 9.0
...
def finalize_sync_dataframe(sync_df: pd.DataFrame, p: Params) -> Tuple[pd.DataFrame, int, int]:
    ...
    Na V31 o primeiro pulso continua sendo usado como origem temporal e, quando
...
def green_guide_mask(frame: np.ndarray, p: Params) -> np.ndarray:
    """Mascara da linha verde/ciano usada como guia da regua na V31."""
    H, W = frame.shape[:2]
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    h = hsv[:, :, 0]
    s = hsv[:, :, 1]
...
def _estimate_ruler_body_from_green_guide(frame: np.ndarray, guide: dict, p: Params) -> dict:
    """Estima o corpo visivel da regua usando a linha verde apenas como ancora em X."""
    H, W = frame.shape[:2]
    ...
    right_exclude = max(6, int(round(0.35 * float(getattr(p, "ruler_green_probe_right_px", 35)))))
    x1 = max(0, gx - left)
    x2 = min(W, max(x1 + 8, gx - right_exclude))
    if x2 <= x1 + 8:
        return {"ok": False, "reason": "ruler_body_probe_invalid"}
    ...
    bright = ((crop_v >= max(145, int(getattr(p, "ruler_pale_v_min", 90)) + 45))
              & (crop_s <= min(120, int(getattr(p, "ruler_pale_s_max", 150))))).astype(np.uint8)
    if bright.size == 0:
        return {"ok": False, "reason": "ruler_body_empty"}

    k_close = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 11))
    k_open = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 5))
    bright = cv2.morphologyEx(bright * 255, cv2.MORPH_CLOSE, k_close, iterations=1)
    bright = cv2.morphologyEx(bright, cv2.MORPH_OPEN, k_open, iterations=1)
    bright = (bright > 0).astype(np.uint8)

    row_ratio = bright.mean(axis=1).astype(float)
    thr = max(0.18, 0.55 * float(np.nanmax(row_ratio)) if np.isfinite(np.nanmax(row_ratio)) else 0.18)
    active = row_ratio >= thr

    gy0 = int(max(0, math.floor(float(guide.get("y_min_px", 0.0)) - 90)))
    gy1 = int(min(H - 1, math.ceil(float(guide.get("y_max_px", H - 1)) + 90)))
    active[:gy0] = False
    active[gy1 + 1:] = False

    runs = []
    start = None
    for i, flag in enumerate(active.tolist() + [False]):
        if flag and start is None:
            ...
            start = None
    if not runs:
        return {"ok": False, "reason": "ruler_body_not_found", "x1": x1, "x2": x2}

    guide_mid = 0.5 * (float(guide.get("y_min_px", 0.0)) + float(guide.get("y_max_px", H - 1)))
    best = None
    for y0, y1 in runs:
        span = y1 - y0 + 1
        mid = 0.5 * (y0 + y1)
        overlap = max(0, min(y1, gy1) - max(y0, gy0) + 1)
        score = span + 0.35 * overlap - 0.08 * abs(mid - guide_mid)
        if best is None or score > best[0]:
            best = (score, y0, y1)

    _, body_top, body_bottom = best
    return {
        "ok": True,
        "reason": "ruler_body_from_bright_region",
        "x1": int(x1),
        "x2": int(x2),
        "top_y": float(body_top),
        "zero_y": float(body_bottom),
        "span": float(body_bottom - body_top),
        "row_ratio_max": float(np.nanmax(row_ratio)) if row_ratio.size else np.nan,
    }


def _estimate_ruler_span_from_green_guide(frame: np.ndarray, guide: dict, p: Params) -> dict:
    """V31: usa diretamente a altura da linha verde como referencia 0..26 mm.

    Nesta versao a linha verde/ciano deve ter sido desenhada sobre todo o trecho
    fisico da regua: topo = 26 mm e base = 0 mm. Assim, a deteccao automatica das
    marcas pretas deixa de ser usada para calibrar a escala.
    """
    H, W = frame.shape[:2]
    guide_x = float(guide.get("x_px", np.nan))
    y_min = float(guide.get("y_min_px", np.nan))
    y_max = float(guide.get("y_max_px", np.nan))

    if not (np.isfinite(guide_x) and np.isfinite(y_min) and np.isfinite(y_max)):
        return {"ok": False, "reason": "green_guide_missing"}

    # A bbox do OpenCV vem como y+h; para desenho/escala, evita estourar o frame.
    top_y = max(0.0, min(float(H - 1), y_min))
    zero_y = max(0.0, min(float(H - 1), y_max))
    span = zero_y - top_y

    min_span = max(8.0, float(getattr(p, "ruler_green_min_height_px", 25)))
    if span <= min_span:
        return {
            "ok": False,
            "reason": "green_guide_height_too_small",
            "x1": int(max(0, round(guide_x - 20))),
            "x2": int(min(W, round(guide_x + 20))),
            "top_y": float(top_y),
            "zero_y": float(zero_y),
            "span": float(span),
            "tick_count": 0,
    ...
    return {
        "ok": bool(px_per_mm > 0),
        "reason": "green_guide_full_height_reference",
        "x1": int(max(0, round(guide_x - 20))),
        "x2": int(min(W, round(guide_x + 20))),
        "top_y": float(top_y),
        "zero_y": float(zero_y),
        ...
        "body_top_y": float(top_y),
        "body_zero_y": float(zero_y),
        "body_span": float(span),
    }


def detect_fixed_ruler(frame: np.ndarray, p: Params) -> dict:
    """Detecta a regua pela linha verde/ciano.

    V31: a linha verde e a propria referencia metrologica. O topo da linha equivale
    a 26 mm e a base equivale a 0 mm. A escala px/mm vem diretamente da altura da
    linha detectada.
    """
    H, W = frame.shape[:2]
    ...
    if span.get("ok", False):
        ...
        return {
            "ruler_ok": True,
            "ruler_reason": "green_guide_full_height_reference",
            "ruler_detection_mode": "green_guide_height_reference",
    ...
    # Fallback V31: sem linha verde valida o suficiente para servir como referencia;
    # ainda retorna o trecho detectado como CHECK para diagnostico visual.
    if guide.get("ok", False) and np.isfinite(float(guide.get("y_min_px", np.nan))) and np.isfinite(float(guide.get("y_max_px", np.nan))):
        ...
        return {
            ...
            "ruler_detection_mode": "green_guide_height_fallback",
...
def detect_ball_candidates(frame: np.ndarray, ruler: dict, p: Params, frame_idx: int = 0) -> Tuple[List[dict], np.ndarray, List[dict], dict]:
    ...
    for component_id, contour in enumerate(contours):
        ...
        min_topo_mm = float(getattr(p, "ball_min_topo_ruler_mm", 9.0))
...
def build_treated_output_dataframe(df: pd.DataFrame) -> Tuple[pd.DataFrame, str, str]:
    ...
    # V31: mantém o CSV tratado enxuto, mas registra quando o LED estava aceso.
...
def process_video(video_path: str, output_dir: str, p: Params):
    ...
    if bool(getattr(p, "historical_x_gate_enabled", True)) and len(df) > 0:
        try:
            min_topo_mm_hist = float(getattr(p, "ball_min_topo_ruler_mm", 9.0))
            good_hist = (
    ...
    df["ball_min_topo_ruler_mm_config"] = float(getattr(p, "ball_min_topo_ruler_mm", 9.0))
    ...
    # V31: o ultimo LED apagando indica fim da rotina; encerra o CSV tratado ali.
    ...
    print("MAGLEV VIDEO TRACKER V31")
...
def make_preview(video_path: str, df: pd.DataFrame, ruler: dict, out_path: str, p: Params):
    ...
    for idx in idxs:
        ...
        if w > 0:
            xL = max(0, x - 20); xR = min(vis.shape[1]-1, x + w + 20)
        else:
            xL = 0; xR = vis.shape[1]-1
        cv2.line(vis, (xL, zy), (xR, zy), (0, 255, 255), 1)
        cv2.line(vis, (xL, ty), (xR, ty), (255, 255, 0), 1)

        gx = ruler.get("ruler_green_guide_x_px", np.nan)
        gy0 = ruler.get("ruler_green_guide_y_min_px", np.nan)
        gy1 = ruler.get("ruler_green_guide_y_max_px", np.nan)
        try:
            if np.isfinite(float(gx)):
                gx_i = int(round(float(gx)))
                if np.isfinite(float(gy0)) and np.isfinite(float(gy1)):
                    cv2.line(vis, (gx_i, int(round(float(gy0)))), (gx_i, int(round(float(gy1)))), (0, 255, 0), 2)
                else:
...
def make_mask_preview(video_path: str, df: pd.DataFrame, ruler: dict, out_path: str, p: Params):
    ...
    for idx in idxs:
        ...
        if row is not None and bool(row.get("valid", False)) and np.isfinite(row.get("bbox_x_px", np.nan)):
            ...
            problema = str(row.get("problema_frame", "INVALID")) if row is not None else "INVALID"
            status = f"INVALID {problema[:40]}"
        txt = f"f={idx} {status} thr={thr:.1f} q={qthr:.1f}"
        scale = float(p.preview_thumb_width_px) / vis.shape[1]
        thumb = cv2.resize(vis, (int(p.preview_thumb_width_px), int(vis.shape[0] * scale)))
        thumb = _add_caption_below(thumb, [txt], font_scale=0.48)
        thumbs.append(thumb)
    cap.release()
    if not thumbs:
        return
...
def _add_caption_below(img: np.ndarray, lines: List[str], font_scale: float = 0.52) -> np.ndarray:
    """Retorna uma copia da imagem com uma faixa de legenda abaixo."""
    clean_lines = [str(x).strip()[:160] for x in lines if str(x).strip()]
    if not clean_lines:
        return img
    line_h = max(20, int(round(22 * max(0.8, font_scale / 0.52))))
    pad_y = 8
    pad_x = 10
    panel_h = pad_y * 2 + line_h * len(clean_lines)
    panel = np.full((panel_h, img.shape[1], 3), 245, dtype=np.uint8)
    for i, text in enumerate(clean_lines):
        y = pad_y + 15 + i * line_h
        _draw_text_with_shadow(panel, text, (pad_x, y), font_scale, (40, 40, 40), 1)
...
def make_ruler_preview(video_path: str, ruler: dict, out_path: str):
    ...
    if w > 0 and h > 0:
        ...
        cv2.line(vis, (x, 0), (x, vis.shape[0]-1), (255, 200, 0), 1)
        cv2.line(vis, (x+w, 0), (x+w, vis.shape[0]-1), (255, 200, 0), 1)
    gx = ruler.get("ruler_green_guide_x_px", np.nan)
    gy0 = ruler.get("ruler_green_guide_y_min_px", np.nan)
    gy1 = ruler.get("ruler_green_guide_y_max_px", np.nan)
    try:
        if np.isfinite(float(gx)):
            gx_i = int(round(float(gx)))
            if np.isfinite(float(gy0)) and np.isfinite(float(gy1)):
                cv2.line(vis, (gx_i, int(round(float(gy0)))), (gx_i, int(round(float(gy1)))), (0, 255, 0), 3)
            else:
                cv2.line(vis, (gx_i, 0), (gx_i, vis.shape[0]-1), (0, 255, 0), 3)
    except Exception:
        pass
    zero_y = int(round(float(ruler.get("ruler_zero_y_px", 0))))
    top_y = int(round(float(ruler.get("ruler_top_y_px", 0))))
    xL = max(0, x - 20) if w > 0 else 0
    xR = min(vis.shape[1]-1, x+w+20) if w > 0 else vis.shape[1]-1
    cv2.line(vis, (xL, zero_y), (xR, zero_y), (0, 255, 255), 2)
    cv2.line(vis, (xL, top_y), (xR, top_y), (255, 255, 0), 2)
    ...
    vis = _add_caption_below(vis, [txt], font_scale=0.58)
...
def make_sync_preview(video_path: str, df: pd.DataFrame, out_path: str, rise_frame: int, fall_frame: int):
    ...
    for f in frames:
        ...
        vis = cv2.resize(vis, (420, int(vis.shape[0] * scale)))
        vis = _add_caption_below(vis, [txt], font_scale=0.50)
        thumbs.append(vis)
    cap.release()
    if not thumbs:
        return
...
def make_plot(df: pd.DataFrame, out_path: str):
    ...
    # V31: desenha todas as bordas do LED, nao apenas a primeira.
    used_labels = set()
    ...
    ax.set_title(f"Maglev video tracking V31 | time_conf={time_conf}")
...
def main():
    ...
    ap.add_argument("--output-dir", default=None, help="Pasta de saida. Default: nome do video + _v31")
    ...
    ap.add_argument("--ball-min-topo-ruler-mm", type=float, default=None, help="Topo mínimo físico aceito para candidato da bolinha. Default: 9.0 mm")
    ...
    output_dir = args.output_dir if args.output_dir else f"{stem}_v31"
