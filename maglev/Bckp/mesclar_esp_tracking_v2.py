...
# -*- coding: utf-8 -*-
"""
MESCLAR ESP + TRACKING V2

Alterações em relação ao mesclar_esp_tracking.py original:
...
- Remove automaticamente os pulsos longos do LED, mantendo os pulsos curtos associados às bordas de subida do PWM.
...
Uso típico:
    python mesclar_esp_tracking_v2.py --esp 2024.csv --tracking 0105_tratado.csv --saida mesclado.csv
"""
...
def merge_files(
    ...
    led_pulses_selected, warnings = select_led_pwm_pulses(
        led_pulses_all,
        expected_count=len(esp_pwm_edges),
        short_max_ms=float(led_short_max_ms),
    )

    n_pairs = min(len(esp_pwm_edges), len(led_pulses_selected))
    if n_pairs <= 0:
        raise ValueError('Não foi possível montar pares PWM rise <-> LED rise.')
    if n_pairs < len(esp_pwm_edges):
        warnings.append(f'Usei apenas {n_pairs} pares de sincronismo para {len(esp_pwm_edges)} bordas PWM detectadas.')

    esp_ref_us = esp_pwm_edges[0]['time_us']
    sync_x = [((esp_pwm_edges[i]['time_us'] - esp_ref_us) / 1000.0) for i in range(n_pairs)]
    ...
    for i in range(n_pairs):
        residual_by_us[int(round(esp_pwm_edges[i]['time_us']))] = (i, residuals[i])
    ...
    return {
        ...
        'esp_primeiro_pwm_rise_us': int(round(esp_pwm_edges[0]['time_us'])),
        'tracking_primeiro_led_pwm_rise_ms': led_pulses_selected[0]['rise_ms'] if led_pulses_selected else None,
...
def main():
    parser = argparse.ArgumentParser(description='Mescla CSV do ESP com tracking tratado do vídeo usando PWM rise <-> LED rise.')
