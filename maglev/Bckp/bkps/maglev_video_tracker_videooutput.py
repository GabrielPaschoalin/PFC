...
# -*- coding: utf-8 -*-
"""
MAGLEV VIDEO TRACKER V24 - SIMPLE DARK-BLOB TOP TRACKER + TRACKING OVERLAY VIDEO
...
Exemplo:
    python maglev_video_tracker_v24.py --video video.mp4 --output-dir saida_v24
"""
...
class Params:
    ...
    ball_min_topo_ruler_mm: float = 8.0
...
def detect_ball_candidates(frame: np.ndarray, ruler: dict, p: Params, frame_idx: int = 0) -> Tuple[List[dict], np.ndarray, List[dict], dict]:
    ...
    for component_id, contour in enumerate(contours):
        ...
        min_topo_mm = float(getattr(p, "ball_min_topo_ruler_mm", 8.0))
...
def process_video(video_path: str, output_dir: str, p: Params):
    ...
    if bool(getattr(p, "historical_x_gate_enabled", True)) and len(df) > 0:
        try:
            min_topo_mm_hist = float(getattr(p, "ball_min_topo_ruler_mm", 8.0))
            good_hist = (
    ...
    df["ball_min_topo_ruler_mm_config"] = float(getattr(p, "ball_min_topo_ruler_mm", 8.0))
    ...
    print("MAGLEV VIDEO TRACKER V24")
    ...
    while True:
        ...
        if row is not None:
            ...
            if valid and np.isfinite(y_top_raw):
                ...
                _draw_text_with_shadow(vis, label, (10, 24), 0.55, (255, 255, 255), 1)
                _draw_text_with_shadow(vis, "laranja=raw | azul=median(plot) | vermelho=ponto medido", (10, 46), 0.48, (230, 230, 230), 1)
            else:
                problema = str(row.get("problema_frame", "INVALID"))[:95]
                _draw_text_with_shadow(vis, f"f={idx} INVALID conf={conf:.2f}", (10, 24), 0.55, (255, 255, 255), 1)
                _draw_text_with_shadow(vis, problema, (10, 46), 0.48, (255, 255, 255), 1)
        else:
            _draw_text_with_shadow(vis, f"f={idx} sem dados no CSV", (10, 24), 0.55, (255, 255, 255), 1)
...
def make_plot(df: pd.DataFrame, out_path: str):
    ...
    ax.set_title(f"Maglev video tracking V24 | time_conf={time_conf}")
...
def main():
    ...
    ap.add_argument("--output-dir", default=None, help="Pasta de saida. Default: nome do video + _v24")
    ...
    ap.add_argument("--ball-min-topo-ruler-mm", type=float, default=None, help="Topo mínimo físico aceito para candidato da bolinha. Default: 8.0 mm")
    ...
    output_dir = args.output_dir if args.output_dir else f"{stem}_v24"
