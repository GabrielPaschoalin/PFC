...
# -*- coding: utf-8 -*-
"""
MAGLEV VIDEO TRACKER V30 - GREEN LINE HEIGHT RULER REFERENCE
...
Exemplo:
    python maglev_video_tracker_v30.py --video video.mp4 --output-dir saida_v30
"""
...
def finalize_sync_dataframe(sync_df: pd.DataFrame, p: Params) -> Tuple[pd.DataFrame, int, int]:
    ...
    Na V30 o primeiro pulso continua sendo usado como origem temporal e, quando
...
def green_guide_mask(frame: np.ndarray, p: Params) -> np.ndarray:
    """Mascara da linha verde/ciano usada como guia da regua na V30."""
    H, W = frame.shape[:2]
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    h = hsv[:, :, 0]
    s = hsv[:, :, 1]
...
def _estimate_ruler_span_from_green_guide(frame: np.ndarray, guide: dict, p: Params) -> dict:
    """V30: usa diretamente a altura da linha verde como referencia 0..26 mm.
...
def detect_fixed_ruler(frame: np.ndarray, p: Params) -> dict:
    ...
    V30: a linha verde e a propria referencia metrologica. O topo da linha equivale
    ...
    # Fallback V30: sem linha verde valida o suficiente para servir como referencia;
...
def build_treated_output_dataframe(df: pd.DataFrame) -> Tuple[pd.DataFrame, str, str]:
    ...
    # V30: mantém o CSV tratado enxuto, mas registra quando o LED estava aceso.
...
def process_video(video_path: str, output_dir: str, p: Params):
    ...
    # V30: o ultimo LED apagando indica fim da rotina; encerra o CSV tratado ali.
    ...
    print("MAGLEV VIDEO TRACKER V30")
...
def make_plot(df: pd.DataFrame, out_path: str):
    ...
    # V30: desenha todas as bordas do LED, nao apenas a primeira.
    used_labels = set()
    ...
    ax.set_title(f"Maglev video tracking V30 | time_conf={time_conf}")
...
def main():
    ...
    ap.add_argument("--output-dir", default=None, help="Pasta de saida. Default: nome do video + _v30")
    ...
    output_dir = args.output_dir if args.output_dir else f"{stem}_v30"
