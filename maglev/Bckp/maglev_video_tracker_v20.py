...
# -*- coding: utf-8 -*-
"""
MAGLEV VIDEO TRACKER V20 - SIMPLE DARK-BLOB TOP TRACKER + FIXED ZERO RULER
...
def detect_ball_candidates(frame: np.ndarray, ruler: dict, p: Params) -> Tuple[List[dict], np.ndarray]:
    ...
    for contour in contours:
        ...
        is_giant = (
            ...
            or (w / max(1.0, float(h))) >= float(getattr(p, "giant_blob_aspect_ratio", 2.60)
            )
        )
...
def process_video(video_path: str, output_dir: str, p: Params):
    ...
    print("MAGLEV VIDEO TRACKER V20")
...
def make_plot(df: pd.DataFrame, out_path: str):
    ...
    ax.set_title(f"Maglev video tracking V20 | time_conf={time_conf}")
...
def main():
    ...
    output_dir = args.output_dir if args.output_dir else f"{stem}_v20"
