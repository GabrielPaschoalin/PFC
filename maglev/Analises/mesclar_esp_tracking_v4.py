...
# -*- coding: utf-8 -*-
"""
MESCLAR ESP + TRACKING V4

Alterações em relação ao mesclar_esp_tracking_v4.py:
...
- Sincroniza usando as bordas de subida por ordem:
    bordas de subida do PWM no CSV do ESP  <->  bordas de subida de led_on no CSV tratado.
- Não descarta o primeiro LED por ele ser longo: para sincronização importa o instante em que led_on vira True,
  não a duração do pulso.
- Se houver mais pulsos de LED do que bordas de PWM, descarta os LEDs extras no final, tipicamente o LED final
  de encerramento da rotina.
...
Uso típico:
    python mesclar_esp_tracking_v4.py --esp 0105.csv --tracking 0105_tratado.csv --saida 0105_mesclado_v4.csv
"""
...
def select_led_pwm_pulses_by_rise_order(
    led_pulses: Sequence[Dict[str, float]],
    expected_count: int,
    led_start_index: int = 0,
) -> Tuple[List[Dict[str, float]], List[str]]:
    """Seleciona pulsos de LED por ordem de borda de subida.

    V4: a largura do pulso nao define se ele e valido para sincronizacao.
    O evento relevante e o instante em que led_on vira True. Assim, o primeiro
    LED longo pode ser pareado com o primeiro PWM, se essa for a primeira borda
    de subida detectada no video.
    """
    warnings: List[str] = []
    if not led_pulses:
        raise ValueError('Não encontrei pulsos de LED no tracking tratado pela coluna led_on.')
    if expected_count <= 0:
        raise ValueError('expected_count precisa ser positivo.')

    led_start_index = int(led_start_index)
    if led_start_index < 0:
        raise ValueError('--led-start-index não pode ser negativo.')
    if led_start_index >= len(led_pulses):
        raise ValueError(
            f'--led-start-index={led_start_index} é inválido: só há {len(led_pulses)} pulsos de LED.'
        )

    candidates = list(led_pulses[led_start_index:])
    if len(candidates) < expected_count:
        warnings.append(
            f'Foram encontrados apenas {len(candidates)} pulsos LED a partir do índice {led_start_index} '
            f'para {expected_count} bordas PWM. Usei pareamento parcial.'
        )
        return candidates, warnings

    if len(candidates) > expected_count:
        warnings.append(
            f'Foram encontrados {len(candidates)} pulsos LED a partir do índice {led_start_index} '
            f'para {expected_count} bordas PWM. Descartei {len(candidates) - expected_count} pulso(s) extra(s) no final.'
        )

    return candidates[:expected_count], warnings
...
def merge_files(
    ...
    if tracking_led_col not in tracking_rows[0]:
        # Compatibilidade com arquivos que tenham outro nome para o pulso de LED/sync.
        found_led_col = None
        for candidate in ['led_on', 'sync_pulse', 'SYNC_PULSE', 'sync', 'SYNC']:
            if candidate in tracking_rows[0]:
                found_led_col = candidate
                break
        if found_led_col is None:
            available = ', '.join(tracking_rows[0].keys())
            raise ValueError(
                f"O arquivo tracking tratado precisa ter uma coluna de LED, preferencialmente 'led_on'. "
                f"Colunas encontradas: {available}. Regere o CSV tratado com o maglev_video_tracker_v31.py "
                f"ou informe --tracking-led-col com uma coluna equivalente."
            )
        tracking_led_col = found_led_col
    ...
    led_pulses_selected, led_warnings = select_led_pwm_pulses_by_rise_order(
        led_pulses_all,
    ...
    return {
        ...
        'pulsos_led_ignorados_inicio': int(led_start_index),
        ...
        'estrategia_sincronismo': f'PWM a partir do índice {sync_start_pwm_index} pareado por ordem com LED a partir do índice {int(led_start_index)}',
...
def main():
    parser = argparse.ArgumentParser(description='Mescla CSV do ESP com tracking tratado do vídeo usando PWM rise <-> LED rise. V4 pareia as bordas por ordem e usa o primeiro LED mesmo se ele for longo.')
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Índice zero-based da primeira borda PWM usada no ajuste. Default=0: usa a primeira borda.',
    )
    parser.add_argument(
        '--led-start-index',
        type=int,
        default=0,
        help='Índice zero-based do primeiro pulso LED usado no ajuste. Default=0: usa o primeiro LED, mesmo se ele for longo.',
    )
    ...
    resumo = merge_files(
        ...
        led_start_index=args.led_start_index,
