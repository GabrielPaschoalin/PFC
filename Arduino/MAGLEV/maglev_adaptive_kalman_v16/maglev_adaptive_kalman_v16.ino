...
// ============================================================
// MAGLEV - KALMAN ADAPTATIVO V16
// ------------------------------------------------------------
// Baseada na V15, com correcoes pedidas:
// - inicializacao do PWM igual a V11
// - PWM manual por valor direto 0..255 no monitor serial
// - parser serial robusto para \n, \r e tambem sem line ending
// - limite do PD ajustado para 170
// - logs reduzidos da V15 mantidos
//
// Log unico:
...
void processarComandoC(const char* comandoBruto) {
  ...
  char cmdWord[8] = {0};
  if (sscanf(comando, "%7s %f %f %f", cmdWord, &a, &b, &c) == 4) {
    if (equalsIgnoreCase(cmdWord, "ON")) {
      ligarControle(a, b, c);
      return;
    }
  }
